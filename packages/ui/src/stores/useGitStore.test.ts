...
describe('useGitStore', () => {
  ...
  test('drops a cached diff when only the working-tree mtime changed', async () => {
    ...
    const stats = { staged: {}, working: { 'src/index.ts': { insertions: 3, deletions: 1 } } };
  ...
  test('keeps a cached diff when the file is untouched', async () => {
    ...
    useGitStore.getState().setDiff('/repo', 'src/index.ts', { original: 'old', modified: 'current', submodule: null });
