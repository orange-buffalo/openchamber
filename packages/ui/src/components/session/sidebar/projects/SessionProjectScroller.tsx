...
function SessionProjectScrollerComponent({ model, view, actions }: Props): React.ReactNode {
  ...
  const renderRow = React.useCallback((row: SessionSidebarRow, index: number): React.ReactNode => {
    ...
    if (row.kind === 'project-header') {
      ...
      return <>
        {index > 0 && !model.singleProjectMode && (
          <div className="mb-3 mt-1 pl-4 pr-3.5" aria-hidden="true">
  ...
  const nestedRowKeys = React.useMemo(() => {
    ...
    let nested = false;
    for (const row of model.rowModel.rows) {
      if (row.kind === 'project-header' || row.kind === 'activity-header') nested = false;
      else if (row.kind === 'group-header') nested = !row.group.isMain;
      if (nested) keys.add(row.key);
    }
    return keys;
  }, [model.rowModel.rows]);
      ...
      >
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={(event) => {
          ...
          <SortableContext items={structuralIds} strategy={verticalListSortingStrategy}>
            <SessionSidebarRows model={model.rowModel} scrollElement={scrollElement} pinnedRowIndexes={pinnedRowIndexes} renderRow={(row, index) => (
              <div className={nestedRowKeys.has(row.key) ? 'pl-5' : undefined}>{renderRow(row, index)}</div>
